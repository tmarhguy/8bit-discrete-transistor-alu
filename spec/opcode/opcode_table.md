...
* `OPCODE[4:0]`: 5-bit operation selector
* `SEL_ALU_SRC`: 0=Arithmetic, 1=Logic (derived from OPCODE[4:3])
* `INV_OUT`: global post-mux inversion bit (derived from OPCODE)
* `LOAD_A/B/R`: latch pulses or level enables

## Opcode Table (19 Operations)

| OPCODE Decimal | OPCODE Binary [4:0] | Operation | Description | Implementation | Circuit Progress |
|----------------|---------------|-----------|-----------|-------------|----------------|------------------|
| 0 | 00000 | ADD | A + B | Arithmetic: A + B | Complete |
| 1 | 00001 | SUB | A - B | Arithmetic: A - B (uses M=1) | Complete |
| 2 | 00010 | INC A | A + 1 | Arithmetic: A + 1 | Complete |
| 3 | 00011 | DEC A | A - 1 | Arithmetic: A - 1 (uses M=1) | Complete |
| 4 | 00100 | LSL / SLL | Shift left logical | Arithmetic: Shift A left by 1 bit | Complete |
| 5 | 00101 | LSR / SRL | Shift right logical | Arithmetic: Shift A right by 1 bit | Complete |
| 6 | 00110 | ASR | Arithmetic shift right | Arithmetic: Shift A right with sign extension | Complete |
| 7 | 00111 | REV A | Reverse A bits | Arithmetic: Reverse bit order of A | Complete |
| 8 | 01000 | NAND | A NAND B | Logic: Base NAND operation (INV_OUT=0) | Complete |
| 9 | 01001 | NOR | A NOR B | Logic: Base NOR operation (INV_OUT=0) | Complete |
| 10 | 01010 | XOR | A XOR B | Logic: Base XOR operation (INV_OUT=0) | Complete |
| 11 | 01011 | PASS A | Pass A through | Logic: Pass A to output unchanged | Complete |
| 12 | 01100 | PASS B | Pass B through | Logic: Pass B to output unchanged | Complete |
| 13 | 01101 | AND | A AND B | Logic: Implemented as NAND + invert (INV_OUT=1) | Complete |
| 14 | 01110 | OR | A OR B | Logic: Implemented as NOR + invert (INV_OUT=1) | Complete |
| 15 | 01111 | XNOR | A XNOR B | Logic: Implemented as XOR + invert (INV_OUT=1) | Complete |
| 16 | 10000 | CMP | Compare (A - B flags only) | Arithmetic: A - B for flags, no output | Complete |
| 17 | 10001 | NOT A | Invert A | Logic: Implemented as PASS A + invert (INV_OUT=1) | Complete |
| 18 | 10010 | NOT B | Invert B | Logic: Implemented as PASS B + invert (INV_OUT=1) | Complete |
...
The control unit decodes `OPCODE[4:0]` into:
...
The opcode is encoded as a 5-bit binary value (0-18 currently used, allowing expansion to 31 operations):

* **Bits [4:3]**: Category selection
  * `00`: Arithmetic & Shift (ADD, SUB, INC, DEC, shifts, REV)
  * `01`: Logic operations (NAND, NOR, XOR, PASS, AND, OR, XNOR)
  * `10`: Comparison and inversion (CMP, NOT A, NOT B)
